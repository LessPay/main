...
/*	onConnect: function(session, socket) {
		...
		if(session.passport.user) {
			UserService.add(session.passport.user, socket);

			session.socket_id = sails.sockets.id(socket);
			session.save();
			...
			User.findOne(session.passport.user).exec(function (err, user) {
	...
	onDisconnect: function(session, socket) {
		...
		if(session.passport.user) {
			UserService.remove(session.passport.user, socket);

			var socketID = sails.sockets.id(socket);

			delete session.socket_id;
			if(socketID in session.chat) delete session.chat[socketID];
